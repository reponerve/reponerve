...
// NewCommand creates the explain-interface subcommand.
func NewCommand() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "explain-interface [symbol]",
		Short: "Explain an interface symbol",
		Long:  `Resolve an interface through Code Intelligence and attach related repository context.`,
	...
	cmd.RunE = func(cmd *cobra.Command, args []string) error {
		return devwire.RunSymbolExplanation(cmd, args[0], pkgFlag, func(ctx context.Context, session *devwire.Handle, symbol, packagePath string) (*development.DevelopmentExplanation, error) {
			return session.Service.ExplainInterface(ctx, session.RepositoryID, symbol, packagePath)
		})
	}
	return cmd
}
